...
export default function DashboardPage() {
  ...
  if (!metrics) return <div className="p-8">{t("noMetrics")}</div>;
  ...
  return (
    <div
      dir={lang === "ar" ? "rtl" : "ltr"}
      className="min-h-screen bg-gradient-to-b from-gray-50 to-gray-100 dark:from-slate-900 dark:to-slate-800 transition-colors duration-200"
    >
      <div className="absolute inset-0 bg-grid-slate-900/[0.04] dark:bg-grid-slate-100/[0.03] -z-10"></div>
      <div className="container mx-auto px-4 py-12 max-w-6xl relative">
        <h1 className="text-5xl sm:text-6xl font-display font-bold bg-clip-text text-transparent bg-gradient-to-r from-indigo-600 via-blue-600 to-cyan-600 dark:from-indigo-300 dark:via-blue-300 dark:to-cyan-300 mb-8 leading-tight tracking-tight">
          {t("dashboard")}
        </h1>
            ...
            <div className="flex items-center gap-6">
              ...
              <div className="flex-1">
                <p className="text-sm text-gray-600 dark:text-gray-300">
                  {t("totalTickets")}: <strong>{metrics.total}</strong>
                </p>
            ...
            <div className="flex items-center gap-6">
              <div style={{ width: 240 }}>
                <SimpleBar
                  ...
                  max={Math.max(...prioData, 1)}
                />
              </div>
              <div className="flex-1">
                <p className="text-sm text-gray-600 dark:text-gray-300">
                  {t("totalPrioritized")}: <strong>{prioTotal}</strong>
                </p>
                <ul className="mt-4 space-y-2">
                  <li className="flex items-center justify-between">
                    <span>{t("high")}</span>
                    <span className="font-medium">{prioHigh}</span>
                  </li>
                  <li className="flex items-center justify-between">
                    <span>{t("medium")}</span>
                    <span className="font-medium">{prioMedium}</span>
                  </li>
                  <li className="flex items-center justify-between">
                    <span>{t("low")}</span>
                    <span className="font-medium">{prioLow}</span>
                  </li>
                </ul>
              </div>
            </div>
        ...
        <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="p-6 backdrop-blur-xl bg-white/50 dark:bg-slate-900/50 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm hover:shadow-md transition-all duration-200">
            ...
            <ol className="list-decimal pl-5 space-y-2 text-sm text-gray-700 dark:text-gray-300">
              {metrics.topIntents.length === 0 && <li>{t("noIntents")}</li>}
              {metrics.topIntents.map((tItem) => (
                <li key={tItem.intent} className="flex justify-between">
                  <span>{tItem.intent}</span>
                  <span className="font-medium">{tItem.count}</span>
                </li>
              ))}
            </ol>
          </div>
          ...
          <div className="p-6 backdrop-blur-xl bg-white/50 dark:bg-slate-900/50 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm hover:shadow-md transition-all duration-200">
            ...
            {metrics.trend.length === 0 ? (
              <div className="text-sm text-gray-700 dark:text-gray-300">{t("noActivity")}</div>
            ) : (
              <div className="space-y-4">
                <div className="flex flex-wrap gap-4 mb-2">
                  <div className="flex-1 min-w-[160px]">
                    <div className="text-xs text-gray-500">{t("avgDay")}</div>
                    <div className="text-2xl font-semibold">
                      {Math.round(
                        metrics.trend.reduce((a, b) => a + b.count, 0) /
                          Math.max(metrics.trend.length, 1)
                      )}
                    </div>
                  </div>
                  <div className="flex-1 min-w-[160px]">
                    <div className="text-xs text-gray-500">{t("avg7")}</div>
                    <div className="text-2xl font-semibold">
                      {compute7DayAvg(metrics.trend)}
                    </div>
                  </div>
                  <div className="flex-1 min-w-[160px]">
                    <div className="text-xs text-gray-500">{t("change7")}</div>
                    <div className="text-2xl font-semibold">
                      {computePercentChange(metrics.trend)}
                    </div>
                  </div>
                </div>
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
      <g transform={`translate(${radius}, ${radius})`}>
        ...
        <text x={0} y={4} textAnchor="middle" fontSize={12} fill="#111" className="dark:text-white">
          {total}
        </text>
      </g>
    </svg>
...
const translations = {
  en: {
    ...
    medium: "Medium",
    high: "High",
    totalPrioritized: "Total prioritized tickets",
    topIntents: "Top intents",
    noIntents: "No intents found",
    trendTitle: "Daily tickets — trend & 7-day average",
    noActivity: "No activity in the last 30 days.",
    avgDay: "Avg / day",
    avg7: "7-day avg",
    change7: "Change vs prev 7d",
  },
  ar: {
    loading: "جارٍ تحميل الإحصائيات...",
    noMetrics: "لا توجد بيانات متاحة",
    dashboard: "لوحة التحكم",
    openClosed: "مفتوحة مقابل مغلقة",
    open: "مفتوحة",
    closed: "مغلقة",
    ...
    medium: "متوسطة",
    high: "عالية",
    totalPrioritized: "إجمالي التذاكر حسب الأولوية",
    topIntents: "أهم النوايا",
    noIntents: "لم يتم العثور على نوايا",
    trendTitle: "التذاكر اليومية — الاتجاه والمتوسط خلال 7 أيام",
    noActivity: "لا يوجد نشاط خلال آخر 30 يومًا.",
    avgDay: "المتوسط اليومي",
    avg7: "متوسط 7 أيام",
    change7: "التغير مقارنة بـ 7 أيام سابقة",
  },
};
